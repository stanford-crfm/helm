...
To run this benchmark using the HELM command-line tools, we need to specify **run entries** that describe the desired runs. For this example, the run entries are `mmlu:subject=anatomy,model=openai/gpt2` (for anatomy) and `mmlu:subject=philosophy,model=openai/gpt2` (for philosophy).
...
After running this command, navigate to the `benchmark_output/runs/my-suite/` directory. This should contain two sub-directories named `mmlu:subject=anatomy,model=openai_gpt2` and `mmlu:subject=philosophy,model=openai_gpt2`. Note that the names of these sub-directories are based on the run entries we used earlier, but with `/` replaced with `_`.
...
Finally, the `helm-server` command launches a web server to visualize the output files generated by `helm-run` and processed by `helm-summarize`. Run:
