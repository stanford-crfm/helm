...
do
    ...
    'helm-run --skip-run-specs-json -n $num_threads --models-to-run $model $default_args $* > $logs_path/$log_file 2>&1'"
    ...
    'helm-run --skip-run-specs-json -n $num_threads --models-to-run $model --conf-path src/helm/benchmark/presentation/run_specs_gpu.conf
...
command="helm-run --skip-instances --models-to-run ${models[@]} $default_args $* > $logs_path/run_all_skip_instances.log 2>&1"
echo "nlprun $default_nlp_run_args --job-name generate-run-specs-json-$suite --priority high -g 0 --memory 8g '$command'"
command="helm-summarize --suite $suite > $logs_path/summarize.log 2>&1"
