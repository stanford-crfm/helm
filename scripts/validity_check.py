...
if __name__ == "__main__":
    ...
    sub_long: pd.DataFrame = long[(long['benchmark'] == benchmark) & (long['scenario']  == scenario)].copy()
    ...
    z: torch.Tensor = torch.zeros(n_questions, requires_grad=True, device=device)
    a: torch.Tensor = torch.ones(n_questions, requires_grad=True, device=device)
    optim: torch.optim.Optimizer = torch.optim.LBFGS([z, a], lr=0.1, max_iter=20, history_size=10, line_search_fn="strong_wolfe")
    thetas: torch.Tensor = torch.randn(150, n_test_takers, device=device)
    def closure():
        optim.zero_grad()
        probs = torch.sigmoid((thetas[:, :, None] + z[None, None, :])*a[None, None, :])
        loss = -(Bernoulli(probs=probs).log_prob(data_tensor)).mean() + 0.01*(a - 1).pow(2).mean()
        loss.backward()
        return loss
