...
export default function Tab({
  ...
  const handleAgreement = () => {
    if (inputRef.current !== null && inputRef.current.value.trim() === "Yes, I agree") {
      onAgree();
    } else {
      alert("Please type 'Yes, I agree' exactly.");
    }
  };
  const agreement = (runName.includes("gpqa") ? <GPQATerms /> :
  runName.includes("ewok") ? <EWoKTerms /> : null)
...
function EWoKTerms() {
  return (<div>
  <p className="mb-4">
        The EWoK dataset instances are encrypted by default to comply with
        the following request:
      </p>
      <blockquote className="italic border-l-4 border-gray-300 pl-4 text-gray-700 mb-4">
