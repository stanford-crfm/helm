...
from helm.proxy.tokenizers.huggingface_tokenizer_utils import HuggingFacePreTrainedTokenizerFactory
...
=======
        tokenizer = HuggingFacePreTrainedTokenizerFactory.get_tokenizer(
            helm_tokenizer_name=tokenizer_name,
            pretrained_model_name_or_path=pretrained_model_name_or_path or tokenizer_name,
        )
        assert tokenizer_name in HuggingFacePreTrainedTokenizerFactory._tokenizers, "Tokenizer should be cached"
        assert len(tokenizer.encode(TestHuggingFaceTokenizer.TEST_PROMPT)) == expected_num_tokens
...
=======
        tokenizer = HuggingFacePreTrainedTokenizerFactory.get_tokenizer(
            "huggingface/gpt2", pretrained_model_name_or_path="gpt2"
        )
        token_ids = tokenizer.encode(eos_token)
        assert singleton(token_ids) == 50256
        assert tokenizer.decode(token_ids) == eos_token
