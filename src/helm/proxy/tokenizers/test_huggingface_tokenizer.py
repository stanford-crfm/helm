...
class TestHuggingFaceTokenizer:
    ...
    def verify_get_tokenizer(tokenizer_name: str, expected_num_tokens: int):
        tokenizer = HuggingFaceTokenizer.get_tokenizer(tokenizer_name)
        assert tokenizer_name in HuggingFaceTokenizer.tokenizers, "Tokenizer should be cached"
        assert len(tokenizer.encode(TestHuggingFaceTokenizer.TEST_PROMPT)) == expected_num_tokens

    def test_get_tokenizer_gpt2(self):
        TestHuggingFaceTokenizer.verify_get_tokenizer("huggingface/gpt2", 51)
    ...
    def test_gpt2_tokenize_eos(self):
        ...
        tokenizer = HuggingFaceTokenizer.get_tokenizer("huggingface/gpt2")
