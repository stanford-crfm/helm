...
_HELM_CREDENTIALS_NAME = "HELM_CREDENTIALS"


@pytest.mark.models
class TestAutoClient:
    def make_request_and_check_result(self, request, expected_result):
        credentials = parse_hocon(os.getenv(_HELM_CREDENTIALS_NAME, ""))
        if not credentials:
            pytest.skip(f"Skipping test because no credentials found in environment variable {_HELM_CREDENTIALS_NAME}")
    ...
    def test_make_request_stabilityai(self):
        request = Request(
