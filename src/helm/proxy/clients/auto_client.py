...
class AutoClient(Client):
    ...
    def get_critique_client(self) -> CritiqueClient:
        ...
        elif critique_type == "mturk" or critique_type == "mturk-sandbox":
            self.critique_client = MechanicalTurkCritiqueClient(
                # Use separate caches for "mturk" and "mturk-sandbox"
                self._build_cache_config(critique_type),
                aws_access_key_id=self.credentials.get("awsAccessKeyId", None),
                aws_secret_access_key=self.credentials.get("awsSecretAccessKey", None),
                aws_session_token=self.credentials.get("awsSessionToken", None),
                endpoint_url=MechanicalTurkCritiqueClient.SANDBOX_ENDPOINT_URL
                if critique_type == "mturk-sandbox"
                else None,
            )
