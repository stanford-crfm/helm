...
def test_truncate_sequence():
    ...
    truncate_sequence_helper(
        ["a", "b", "c"],
        Request(model_deployment="openai/text-davinci-002", prompt="abc", echo_prompt=True),
        ["a", "b", "c"],
    )
    ...
    truncate_sequence_helper(
        ["hello", " world"],
        Request(model_deployment="openai/text-davinci-002", stop_sequences=["#"]),
        ["hello", " world"],
    )
    ...
    truncate_sequence_helper(
        ["a", "b", "c"], Request(model_deployment="openai/text-davinci-002", max_tokens=2), ["a", "b"]
    )
