...
class AI21Client(CachingClient):
    ...
    def make_request(self, request: Request) -> RequestResult:
        ...
        def parse_sequence(raw: Dict, first: bool, finish_reason: Optional[Dict] = None) -> Completion:
            text = raw["text"]
            tokens = [parse_token(token, first and i == 0) for i, token in enumerate(raw["tokens"])]
            logprob = sum(token.logprob for token in tokens)
            return Completion(text=text, logprob=logprob, tokens=tokens, finish_reason=finish_reason)
