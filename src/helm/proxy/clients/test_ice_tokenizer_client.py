...
class TestICETokenizerClient:
    def setup_method(self, method):
        ...
        self._tokenizer: str = "TsinghuaKEG/ice"
    ...
    def test_tokenize(self):
        request = TokenizationRequest(text=self.test_prompt, tokenizer=self._tokenizer)
    ...
    def test_encode(self):
        request = TokenizationRequest(text=self.test_prompt, encode=True, tokenizer=self._tokenizer)
        result: TokenizationRequestResult = self.client.tokenize(request)
    ...
    def test_encode_with_truncation(self):
        ...
        request = TokenizationRequest(
            text=self.test_prompt, tokenizer=self._tokenizer, encode=True, truncation=True, max_length=max_length
        )
        result: TokenizationRequestResult = self.client.tokenize(request)
        assert result.raw_tokens == self.encoded_test_prompt[:max_length]

    def test_decode(self):
        request = DecodeRequest(tokens=self.encoded_test_prompt, tokenizer=self._tokenizer)
