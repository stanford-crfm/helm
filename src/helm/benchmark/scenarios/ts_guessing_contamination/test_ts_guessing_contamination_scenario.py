...
from helm.benchmark.scenarios.ts_guessing_contamination.ts_guessing_contamination_scenario import (
    TSGuessingContaminationScenario,
)
...
def test_contamination_ts_guessing_multichoice_scenario():
    with TemporaryDirectory() as tmpdir:
        ...
        scenario = TSGuessingContaminationScenario(
            dataset="bluex", strategy="ts_guessing_question_multichoice", language="pt"
        )
...
def test_contamination_ts_guessing_base_scenario():
    with TemporaryDirectory() as tmpdir:
        ...
        scenario = TSGuessingContaminationScenario(dataset="bluex", strategy="ts_guessing_question_base", language="pt")
        instances = scenario.get_instances(tmpdir)
...
def test_contamination_invalid_strategy():
    with TemporaryDirectory() as tmpdir:
        ...
        scenario = TSGuessingContaminationScenario(dataset="bluex", strategy="invalid_strategy", language="pt")
