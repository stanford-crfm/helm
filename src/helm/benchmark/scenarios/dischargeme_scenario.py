...
def file_preprocessing(data_path: str, task_objective: str) -> pd.DataFrame:
    ...
    Use command to download: wget -r -N -c -np --user {PHYSIONET_USERNAME} --ask-password https://physionet.org/files/discharge-me/1.3/
    data_path is directory that contains the downloaded files: '{base_dir}/physionet.org/'
    """
    # Load the first CSV file
    df_diagnosis = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/diagnosis.csv.gz', compression='gzip', keep_default_na=False)
    df_discharge = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/discharge.csv.gz', compression='gzip', keep_default_na=False)
    df_target = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/discharge_target.csv.gz', compression='gzip', keep_default_na=False)
    df_radiology = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/radiology.csv.gz', compression='gzip', keep_default_na=False)
    df_ed = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/edstays.csv.gz', compression='gzip', keep_default_na=False)
    df_triage = pd.read_csv(f'{data_path}/files/discharge-me/1.3/test_phase_1/triage.csv.gz', compression='gzip', keep_default_na=False)
    df_diagnosis_triage = pd.merge(df_diagnosis, df_triage, on='subject_id', how='inner', suffixes=('_df_diagnosis', '_df_triage'))
    df_diagnosis_triage_discharge = pd.merge(  df_diagnosis_triage, df_discharge, on='subject_id', how='inner', suffixes=('', '_df_discharge'))
    df_diagnosis_triage_discharge_radiology = pd.merge(df_diagnosis_triage_discharge, df_radiology, on='hadm_id', how='inner', suffixes=('', '_df_radiology'))

    df_features = pd.merge(df_diagnosis_triage_discharge_radiology, df_ed, on='hadm_id', how='inner', suffixes=('', '_df_ed'))
...
class DischargeMeScenario(Scenario):
    """
    DischargeMe is a discharge instruction generation dataset and brief hospital course generation datasetcollected from MIMIC-IV data. 
    ...
    Sample Synthetic Prompt:
        ...
        Discharge Text:
        Name: {Patient Name} Unit No: {Unit Number} Date of Birth: {DOB} Date of Admission: {DOA} Date of Discharge: {DOD} 
    ...
    @inproceedings{Xu_2024,
        ...
        author={Xu, Justin and Delbrouck, Jean-Benoit and Johnston, Andrew and and Blankemeier, Louis and Langlotz, Curtis},
        year={2024}
    }
    """
    name = "dischargeme"
    ...
    def get_instances(self, output_path: str) -> List[Instance]:
        ...
        for i in range(df_bhc.shape[0]):
            prompt_bhc = create_prompt(df_bhc.iloc[i]['text'], df_bhc.iloc[i]['text_df_radiology'], 'Brief Hospital Course')
            prompt_di = create_prompt(df_di.iloc[i]['text'], df_di.iloc[i]['text_df_radiology'], 'Discharge Instructions')
