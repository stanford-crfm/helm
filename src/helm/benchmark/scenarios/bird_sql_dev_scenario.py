...
from helm.benchmark.scenarios.bird_sql_dev_scenario_helper import cot_wizard, generate_comment_prompt, generate_schema_prompt
...
class BIRDSQLDevScenario(Scenario):
    ...
    def get_instances(self, output_path: str) -> List[Instance]:
        ...
        ensure_file_downloaded("https://bird-bench.oss-cn-beijing.aliyuncs.com/dev.zip", data_root_path, unpack=True, unpack_type="unzip")
        ...
        # Note: Zip file contains .DS_Store file at the root, which causes dev_databases to unzip into a nested directory
        ...
        for row in dataset:
            ...
            database_name: str = row["db_id"]
            question: str = row["question"]
            knowledge: str = row["evidence"]
            gold_sql: str = row["SQL"]

            schema_prompt = database_schema_prompts[database_name]
            ...
            instance = Instance(
                ...
                references=[
                    Reference(output=Output(text=gold_sql), tags=[CORRECT_TAG])
                ],
                split=VALID_SPLIT,
            )
