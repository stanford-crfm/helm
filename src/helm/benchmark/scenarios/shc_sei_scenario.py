...
class SHCSEIMedScenario(Scenario):
    ...
    """
    name = "shc_sei_med"
    description = "A dataset that classifies whether a clinical note contains a clinician recommendation for parent training in behavior management, which is the first-line evidence-based treatment for young children with ADHD."
    ...
    def create_benchmark(self, csv_path)->Dict[str, str]:
        ...
        with open(csv_path, 'r') as file:
            ...
            for row in reader:
                ...
                context = row['context']
                answer = row['label']
                prompt = f"You are reviewing a clinical note from health records of children with attention deficit hyperactivity disorder (ADHD). Given the following definitions: side Effects Inquiry (SEI): Explicit documentation by the clinician asking about current side effects related to ADHD medications that the child is taking or documentation of specific ADHD medication side effects experienced by the patient. SEI does *not* include future side effects monitoring, such as documentation of potential ADHD medication side effects, including planning to follow patients to monitor side effects, explaining about potential side effects of an ADHD medication. These documentations are not categorized as SEI because they consist of a plan or an explanation about side effects without actual side effect monitoring taking place, and No Side Effects Inquiry (NSEI): No evidence of side effects monitoring. Provide an answer to the following {question} with the following context: {context} , Answer the question with a 'A' for yes or 'B' for no. Do not provide any additional details or response, just a simple A or B response."
                data[prompt] = answer
        return data
