...
if __name__ == "__main__":
    ...
    os.makedirs('./n2c2_ct_matching_stats', exist_ok=True)
    for t in LONG_DEFINITIONS.keys():
        # Skip if already exists
        if os.path.exists(f'./n2c2_ct_matching_stats/{t}.txt'):
            print(f"Skipping {t} because it already exists")
            continue

        # Create benchmark
        scenario = N2C2CTMatchingScenario(subject=t)
        instances = scenario.get_instances('test.csv')
            
        # Calculate prompt token stats
        df = pd.DataFrame({
            'prompt' : [instance.input.text for instance in instances],
            'split': [instance.split for instance in instances],
            'label' : [1 if instance.references[0].output.text == "yes" else 0 for instance in instances]
        })
        df['prompt_n_tokens'] = df['prompt'].progress_apply(lambda x: len(tokenizer.encode(x)))
        with open(f'./n2c2_ct_matching_stats/{t}.txt', 'w') as f:
            f.write("-"*100 + "\n")
            f.write(f"Task: {t}\n")
            f.write(f"# of instances: {len(instances)}\n")
            f.write(f"# of positives: {df['label'].sum()}\n")
