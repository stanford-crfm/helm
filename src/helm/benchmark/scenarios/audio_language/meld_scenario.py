...
class MELDScenario(Scenario):
    ...
    CSV_URL_PREFIX = "https://raw.githubusercontent.com/declare-lab/MELD/2d2011b409d3ca2d7e94460cd007d434b1d0a102/data/MELD/"
    SPLIT_NAME_TO_CSV_FILE_NAME = {VALID_SPLIT: "dev_sent_emo.csv", TRAIN_SPLIT: "train_sent_emo.csv", TEST_SPLIT: "test_sent_emo.csv"}
    SPLIT_NAME_TO_TGZ_FILE_NAME = {VALID_SPLIT: "audios_validation.tgz", TRAIN_SPLIT: "audios_train.tgz", TEST_SPLIT: "audios_test.tgz"}
    SAMPLE_RATE = 16000
    ...
    def get_instances(self, output_path: str) -> List[Instance]:
        ...
        csv_file_name = MELDScenario.SPLIT_NAME_TO_CSV_FILE_NAME[split_name]
        csv_file_path = os.path.join(csv_dir, csv_file_name)
        ensure_file_downloaded(
            MELDScenario.CSV_URL_PREFIX + csv_file_name,
            csv_file_path
        )
        ...
        ensure_file_downloaded(
            source_url=f"https://huggingface.co/datasets/zrr1999/MELD_Text_Audio/resolve/main/archive/{MELDScenario.SPLIT_NAME_TO_TGZ_FILE_NAME[split_name]}?download=true",
        ...
        for row in df.itertuples():
            ...
            if not os.path.isfile(wav_file_path):
                ...
                audio_array = get_array_from_audio_file(flac_file_path)
                ensure_wav_file_exists_from_array(wav_file_path, audio_array, 16_000)
            input = Input(
                multimedia_content=MultimediaObject(
                    media_objects=[MediaObject(location=wav_file_path, content_type="audio/wav")]
                )
            )
            assert row.Emotion in MELDScenario.LABEL_NAMES
