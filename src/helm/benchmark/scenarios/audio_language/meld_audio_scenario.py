...
class MELDAudioScenario(Scenario):
    ...
    def get_instances(self, output_path: str) -> List[Instance]:
        ...
        ensure_file_downloaded(
            MELDAudioScenario.CSV_URL_PREFIX + csv_file_name,
            csv_file_path
        )
        ...
        ensure_file_downloaded(
            source_url=f"https://huggingface.co/datasets/zrr1999/MELD_Text_Audio/resolve/main/archive/{MELDAudioScenario.SPLIT_NAME_TO_TGZ_FILE_NAME[split_name]}?download=true",
        ...
        for row in tqdm(df.itertuples()):
            ...
            instance = Instance(id=str(f"awoo{row.serial_number}"), input=input, references=references, split=split_name)
