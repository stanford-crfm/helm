    ...
    def create_benchmark(self, csv_path)->Dict[str, str]:
        ...
        with open(csv_path, 'r') as file:
            ...
            for row in reader:
                ...
                context = row['context']
                answer = row['label']
                prompt = f" {counter} Provide an answer to the following {question} with the following context: {context} , Answer the question with a 'A' for yes or 'B' for no. Do not provide any additional details or response, just a simple A or B response."
                data[prompt] = answer
                counter += 1
        return data
