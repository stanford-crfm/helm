from .grammar import Grammar, GrammarRule, Expansion, read_grammar, ROOT_CATEGORY, generate_derivations, get_values, get_tags


def test_generate():
    grammar = Grammar(rules=[
        GrammarRule(category=ROOT_CATEGORY, expansions=[
            Expansion(text="hello"),
            Expansion(text="hi ${Name} and ${Name}"),
        ], tags=['t1']),
        GrammarRule(category="Name", expansions=[
...
def test_read_grammar():
    grammar = read_grammar("src/helm/benchmark/scenarios/assistant_prompts.yaml")
