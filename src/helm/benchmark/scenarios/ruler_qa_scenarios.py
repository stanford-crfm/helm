...
from helm.benchmark.scenarios.ruler_qa_scenario_helper import generate_samples_for_helm
...
Question: {query} Answer:"""
    ...
    def __init__(self, dataset: str = "hotpotqa"):
        super().__init__()
    ...
    def get_instances(self, output_path: str) -> List[Instance]:
        ...
        samples = generate_samples_for_helm(dataset_path=file_path, tokenizer="cl100k_base", max_seq_length=32768, tokens_to_generate=32, num_samples=500, random_seed=42, dataset=self.dataset, pre_samples=0, template=self._TEMPLATE)
        for sample in samples:
            instance = Instance(
                id=sample["index"],
                input=Input(text=sample["input"]),
                references=[Reference(Output(text=output_text), tags=[CORRECT_TAG]) for output_text in sample["outputs"]],
                split=VALID_SPLIT,
            )
