...
def define_rules(target_specialty: str):
    rules = []
    rules.append(f"""- All data included from the notes, which is relevant for a specialty of {target_specialty}, is in the summary.""")
    rules.append(f"""- All assertions can be traced back to the notes; NEVER include assertions which cannot be traced back to the notes.""")
    rules.append(f"""- Information from the notes which is pertinent for a specialty of {target_specialty}, or potentially pertinent for a specialty of {target_specialty}, is NEVER omitted.""")
    rules.append(f"""- Information from the notes which is NOT pertinent for a specialty of {target_specialty} IS omitted from the summary.""")
    rules.append(f"""- The level of detail must be appropriate for a reader with a specialty of {target_specialty}.""")
    rules.append(f"""- All assertions must be made with logical order and grouping (temporal or systems/problem based).""")
    rules.append(f"""- Summary must be comprehensible, using plain language that is completely familiar and well-structured for a reader with a specialty of {target_specialty}.""")
    rules.append(f"""- All assertions are captured with fewest words possible and without any redundancy in syntax or semantics.""")
    rules.append(f"""- Where applicable, go beyond relevant groups of events and generate reasoning over the events into a summary that is fully integrated for an overall clinical synopsis with prioritized information.""")
    ...
    rules.append(f"""- For every event (e.g., medicine change, new diagnosis, etc.) mentioned in your summary, mention WHEN it happened (communicate the timing of events) if that information is available in the note.""")
    rules.append(f"""- If it's unclear WHEN an event happened in the notes, instead explain that the event was mentioned by a note written at [timestamp of the note].""")
    rules.append(f"""- For each SENTENCE in the summary, cite the <Note ID> source in the summary using the format <Note ID:IDVAL>, where IDVAL is the ID of the note.""")
    rules.append(f"""- Cite each note tag individually; when citing multiple notes, use the format <Note ID:IDVAL>, <Note ID:IDVAL>.""")
    ...
    rules.append(f"""- It is CRITICALLY IMPORTANT that you cite information to the note it came from! Wrongful citations are HARMFUL!""")
    return rules
