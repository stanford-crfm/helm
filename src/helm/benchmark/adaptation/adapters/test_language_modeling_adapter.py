...
class TestLanguageModelingAdapter(TestAdapter):
    ...
    def test_truncation_of_prefix_and_suffix(self):
        # Test that the prefix and suffix do not get truncated when the prompt is too long
        prefix: str = "This is a prefix\n"
        suffix: str = "A suffix just to make sure the prompt is long enough\n\n"
        adapter_spec = AdapterSpec(
            method=ADAPT_LANGUAGE_MODELING,
            input_prefix=prefix,
            model="anthropic/claude-v1.3",
        ...
        # Step 1.1. Check that if the prompt is not too long, the prefix and suffix are unchanged
        input_text: Input = Input(text="Excuse me, do you have the time?")
        reference: Reference = Reference(output="Yes, it's 12:30.", tags=[])
        instance: Instance = Instance(
            input=input_text,
        ...
        request: Request = request_states[0].request
        # The prompt should be "<|endoftext|>Excuse me, do you have the time?"
        len_eot: int = len("<|endoftext|>")
        assert request.prompt[len_eot : len_eot + len(prefix)] == prefix
        assert request.prompt[-len(suffix) :] == suffix

        # Step 1.2. Check that if the prompt is too long, it is truncated but the prefix and suffix are unchanged
        input_text_long: Input = Input(text="Excuse me, do you have the time? " * 1000)
        instance_long: Instance = Instance(
            input=input_text_long,
            references=[reference],
        ...
        num_tokens = len(adapter.window_service.encode(request_long.prompt).token_values)
        assert num_tokens == adapter.window_service.max_request_length
        assert request_long.prompt[len_eot : len_eot + len(prefix)] == prefix
        assert request_long.prompt[-len(suffix) :] == suffix
