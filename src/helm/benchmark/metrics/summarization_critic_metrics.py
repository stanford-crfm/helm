...
from helm.common.critic_request import CriticTaskTemplate, CriticQuestionTemplate, CriticRequest


class SummarizationCriticMetric(Metric):
    """Reimplementation of SummarizationMetric's evals using critic evaluation.

    This is a demonstration of critic evaluation and is not indended for production use."""

    _TEMPLATE = template = CriticTaskTemplate(
        name="HELM Summarization Evaluation",
        # Note: Instructions can contain HTML.
        instructions="Rate the following summary.\n\nOriginal text:\n{{original_text}}\n\nSummary: {{summary}}",
        num_workers=2,
        questions=[
            CriticQuestionTemplate(
    ...
    def __repr__(self):
        return "SummarizationCriticMetric()"
    ...
    def evaluate_generation(
        ...
        """Ask critics to rate a summarization."""
        assert request_state.result is not None
        if len(request_state.result.completions) != 1:
            raise ValueError("SummarizationCriticMetric only supports a single generation per instance")
        ...
        result = metric_service.make_critic_request(request)
