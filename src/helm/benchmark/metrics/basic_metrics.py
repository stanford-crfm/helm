...
class BasicMetric(Metric):
    ...
    def evaluate_references(
        ...
        def compute_logprob_and_length(request_state: RequestState, window_service: WindowService) -> ReferenceStat:
            ...
            completion: GeneratedOutput = request_state.result.completions[0]
...
def _compute_finish_reason_metrics(
    ...
    if completion.finish_reason is None or completion.finish_reason["reason"] not in valid_reasons:
        reason = "unknown"
    else:
        reason = completion.finish_reason["reason"]
    return [
...
def compute_language_modeling_metrics(
    ...
    tokens: List[Token]
    if request_state.num_conditioning_tokens > 0 and completion.tokens[0].text == "":
        tokens = completion.tokens[1:]
    else:
