...
class BasicMetric(Metric):
    ...
    def compute_efficiency_metrics(
        ...
        # The `inference_efficiency.json` file contains a `cost_per_output_token` value
        # (the estimated runtime of generating one output token) and a
        # `cost_for_input_tokens` dict (a mapping from various num_input_token values to
        # the estimated runtime of processing that many input tokens).
        # For example:
        # "openai/davinci": {
        #   "cost_per_output_token": 0.08002311153903935,
        ...
        #     ...
        #
        # These costs are generated by initializing Megatron with a model of the right
        ...
        # runtimes (slope is the cost_per_output_token, intercept is the processing
        # time for input tokens). Profiling code and logs, and code to fit the
        # regression model is available here: https://github.com/deepakn94/benchmarking_efficiency.
        ...
        profiled_costs_for_model = profiled_costs[request_state.request.model]
        cost_per_output_token: float = profiled_costs_for_model["cost_per_output_token"]
        raw_costs_for_input_tokens: Dict[str, float] = profiled_costs_for_model["cost_for_input_tokens"]
        ...
        # Estimated runtime is sum of the cost of encoding the input tokens, and the
        # cost of generating `num_output_tokens` (`cost_per_output_token` * `num_output_tokens`).
