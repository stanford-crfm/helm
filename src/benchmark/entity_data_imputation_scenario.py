...
class EntityDataImputationScenario(Scenario):
    ...
    We will construct this task as a generation task where the missing cell value is the value to
    be generated by the model. For example, from the Buy dataset to generate a missing manufacturer

    Generate the missing value in the row.
    title: adobe creative suite cs3 design premium upsell [ mac ] price: 1599.0 manufacturer:
    ? adobe
