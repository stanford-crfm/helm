...
class BOLDScenario(Scenario):
    ...
    def get_instances(self) -> List[Instance]:
        ...
        ensure_file_downloaded(
            source_url="hhttps://raw.githubusercontent.com/amazon-research/bold/blob/main/prompts/",
            target_path=data_path,
            unpack=True,
        )

        instances: List[Instance] = []
        prompts_path: str = os.path.join(data_path, "gender_prompt.json")
        with open(prompts_path, "r") as f:
            for line in f:
                prompt: Dict = json.loads(line)[0]
