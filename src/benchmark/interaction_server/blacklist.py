...
def remove_punc(text: str, keep: List[str] = [], replace_with_space: List[str] = ["-", "/"]):
    ...
    punc_table = {
        codepoint: replace_str if chr(codepoint) not in replace_with_space else " "
        for codepoint, replace_str in punc_table.items()
    }
...
def contains_phrase(
    ...
    lowercase_phrases: bool = True,
    remove_punc_text: bool = True,
    remove_punc_phrases: bool = True,
...
def contains_offensive(text: str, blacklist: Set[str], blacklist_max_len: int) -> bool:
    ...
    for text in texts:
        if contains_phrase(
            ...
            max_phrase_len=blacklist_max_len,
        ):
            return True

    # dealing with false positives for "He'll"
    if contains_phrase(
        text.translate({ord(p): "" for p in string.punctuation}),
        blacklist - {"hell"},
        lowercase_text=False,
        lowercase_phrases=False,
        remove_punc_text=False,
