...
class AuthenticatedPerspectiveAPIClient(PerspectiveAPIClient):
    ...
    def _create_request(self, text: str) -> HttpRequest:
        ...
        return self._client.comments().analyze(body=analyze_request)
    ...
    def get_toxicity_attributes_batch(self, text_batch: List[str]) -> Dict[str, ToxicityAttributes]:
        ...
        text_to_responses: Dict[str, Dict] = dict()
        ...
        def callback(request_id, response, exception):
            ...
            text_to_responses[request_id] = response
        ...
        for text in set(text_batch):
            request: HttpRequest = self._create_request(text)
            response, cached = self._cache.get(request.body)

            # If the response is already cached, just updated responses. Otherwise, add the request to the batch request
            if cached:
                text_to_responses[text] = response
            else:
                text_to_cache_key[text] = request.body
                batch_request.add(
                    request=request, request_id=text, callback=callback,
                )
        ...
        for text, response in text_to_responses.items():
    ...
    def get_toxicity_attributes(self, text: str) -> ToxicityAttributes:
        ...
        analyze_request: HttpRequest = self._create_request(text)

        try:

            def do_it():
                with self._request_lock:
                    return analyze_request.execute()

            response, _ = self._cache.get_or_compute(analyze_request.body, do_it)
